...
# ---------------------------------------------------------------------------
# Model defaults  (tuned on GROUP_CLEAN)
# ---------------------------------------------------------------------------
A_DEFAULT          = 10.0
B_DEFAULT          = 0.0
C_DEFAULT          = 4.0
PHI_GEOM_DEFAULT   = 64.5
PHI_INT_DEFAULT    = 40.0
PHI_Z3_DEFAULT     = 72.0
Z3_SKEW_DEFAULT    = 10.0
...
def predict_final(
    ...
    force_branch=None,          # None -> pick best of T2/T3 for heavy sector
                                # 'T2', 'T3', or 'none' -> override
) -> dict:
    """
    ...
    if not heavy_q or force_branch == 'none':
        ...
        # Default to T2; caller can compare both
        amp_heavy  = amp_T2
        branch     = "T2"
        final_mass = final_T2
...
def run_rows(rowspec, best_branch=True, **kwargs) -> list:
    ...
    for name, quarks, J, obs in rowspec:
        pred = predict_final(quarks, J, **kwargs)
