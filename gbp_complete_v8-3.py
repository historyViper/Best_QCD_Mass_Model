#!/usr/bin/env python3
"""
gbp_complete_v8.py — Geometric Boundary Projection Framework v7.7
=============================================================================
...
V7.7 CHANGES FROM V7.6:
  ...
  FREE PARAMETER STATUS (v7.7):
    ...
    *** ZERO FREE PARAMETERS ***
    V8: constituent masses derived from GBP geometry via gbp_quarks_v2.py
...
BARYON_CLASS = {
    ...
    'Sigma_b0'    : ('S2', +1, 'sigma',  2, 'T1',   'heavy'),
    ...
    'P_c(4312)'   : ('S1', -1, 'sigma',  1, 'T1',   'pentaquark'),
    'P_c(4440)'   : ('S1', -1, 'sigma',  1, 'T1',   'pentaquark'),
    'P_c(4457)'   : ('S1', -1, 'sigma',  1, 'T1',   'pentaquark'),
    'P_cs(4459)'  : ('S1', -1, 'lambda', 1, 'T1',   'pentaquark'),
}
...
_PENTA    = {"P_c(4312)","P_c(4440)","P_c(4457)","P_cs(4459)"}
def fit_group(name):
...
def predict_final(quarks, J, name=None):
    ...
    if rule == 'pentaquark':
        ...
        # Full 5-body sumC: all constituent masses (antiquark = full mass)
        # The 2/3 complementary projection (anticharm loses 2/3) absorbed into dg
        # dg uses baryon (3-quark) geo_factor — antiquark dg cancels by symmetry
        # winding sum: c̄c pair cancels → same topology as underlying baryon
        # gc uses standard geo_corr(baryon quarks) — multi-toroid overlap gc
        # is NOT fixed to specific lanes (toroids can overlap in any orientation)
        # Four P_c states differ by toroid overlap angle — pending Hamiltonian paper
        # P_c(4312)=-2.5%, P_c(4440/4457) need orbital splitting on top
        # quarks list has baryon quarks only; antiquark (hidden charm) is implicit
        # add anticharm constituent mass explicitly
        sumC_quarks = sum(CONSTITUENT[q] for q in quarks if q in CONSTITUENT)
        anti_flavor = 'charm'  # hidden charm pentaquark — c̄ is implicit
        sumC_5 = sumC_quarks + CONSTITUENT.get(anti_flavor, 0.0)
        dg_bar = geo_sign * ALPHA_BARYON * LAMBDA_QCD * gf
        final  = (sumC_5 + dg_bar + gc + C_HYP*S) * (1 + lam)
