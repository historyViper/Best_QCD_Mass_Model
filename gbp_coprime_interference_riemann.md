...
| Z₃₀* arithmetic avg | 7/16 (exact) | 1/2 − c₃₀(2)/16 — Ramanujan vacuum defect [†] |
| c₃₀(2) | 1 (exact) | Ramanujan sum μ(15)×φ(30)/φ(15) = 1 |
| Vacuum defect δ | 1/16 = 1/φ(30)² | Oscillating correction to critical line balance |

**[†] Footnote — Z₃₀* arithmetic average vs continuum average (added v2, May 2026):**

Throughout this paper and in the GBP Maxwell paper (v3.3 and earlier), the statement "P(r) averages to 1/2 over Z₃₀*" is correct in the continuum/spatial sense — physical measurements at scales L ≫ lattice spacing a average over many winding cycles, and the Riemann-Lebesgue lemma guarantees the oscillating AC term vanishes in that limit, leaving exactly 1/2. This is the correct statement for the field theory continuum limit.

However, the *arithmetic* average of P(r) over the 8 Z₃₀* labels is:

$$\frac{1}{8}\sum_{r \in Z_{30}^*} \sin^2\!\left(\frac{r\pi}{15}\right) = \frac{7}{16} \neq \frac{1}{2}$$

The deviation 1/2 − 7/16 = 1/16 = 1/φ(30)² is the **Ramanujan vacuum defect**, given exactly by the Ramanujan sum c₃₀(2) = μ(15) × φ(30)/φ(15) = 1. This is not an error in the earlier papers — it is a new result that adds precision. The RH argument in §6 step 5 rests on P(r) ≥ 0, not on the average being 1/2, so it is completely unaffected.

The practical consequence: the small oscillating residual in Riemann zero positions that was previously described as an "O(a/L) finite-modulus correction" is now identified precisely as the Ramanujan defect c₃₀(2)/φ(30)² = 1/64. It is not a vague approximation error — it has an exact value and a geometric origin in the non-isotropy of the Z₃₀* discrete lattice. The GBP Maxwell paper v4.0 (§8.5) contains the full derivation.
...
*GBP/TFFT Framework — April 2026 — v2 (added Ramanujan vacuum defect footnote, May 2026)*
