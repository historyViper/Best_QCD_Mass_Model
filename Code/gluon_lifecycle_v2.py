...
HAMILTONIAN NOTE (added v8.9, April 2026):
  The projection formula above is the diagonal element of the correct
  winding Hamiltonian H = diag(P(r) × Λ_QCD / LU).

  The energy carried by a gluon on lane r is E(r) = P(r) × Λ_QCD/LU.
  This is an eigenvalue equation: H|r⟩ = E(r)|r⟩.

  IMPORTANT: The outer product matrix T_ij = P(r_i) × P(r_j) is NOT
  the correct spectral gap operator — it is rank-1 and cannot demonstrate
  a gap between the vacuum and first excited state. It represents Noether
  charge overlap (useful for Q8), not Hamiltonian energetics.

  The correct spectral gap operator is the diagonal H above.
  The spectral gap Δ = P_min × Λ_QCD/LU = α_IR × Λ_QCD = 184.2 MeV.
  Verified in: gbp_hilbert_spectral_gap.py
