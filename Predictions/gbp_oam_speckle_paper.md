...
The T4 double-helix photon has a figure-eight topology — two loops, each traversing 360°, for 720° total closure. Critically, the two loops are not mirror images of each other. They occupy **opposite chirality Hilbert spaces**: G=+1 (e1 sector) and G=−1 (e2 sector). These are genuinely orthogonal — states in one Hilbert space cannot be expressed as combinations of states in the other without crossing the vacuum seam at 84°.

When the entangled vortex pair propagates through the T1 MMF, the two chirality Hilbert spaces hit the Z₃₀* boundary differently:

- **G=+1 (e1 chirality):** impacts lanes {7, 13, 19} — the mod3=1 colored lanes
- **G=−1 (e2 chirality):** impacts lanes {11, 17, 23} — the mod3=2 colored lanes  
- **Colorless boundary {1, 29}:** shared — accessible from both loops at the closure point

This gives 3 + 3 + 2 = **8 distinct lane impacts**, each genuinely independent because the two sides of the figure-eight land on *different* primes due to chirality. They are not redundant — they are complementary, together tiling Z₃₀* completely.

The 16 sampling points follow directly: the two chirality Hilbert spaces are orthogonal, so you cannot mix measurements between them to reduce the count. Each of the 8 lanes must be sampled once per chirality sector — **8 lanes × 2 chirality Hilbert spaces = 16 independent measurements**. This is not a topological abstraction; it is counting real physical boundary impacts from two orthogonal faces of the figure-eight.

This is why Wang et al. use two orthogonally polarized input beams — LCP excites one chirality sector, RCP excites the other. The experimental setup is directly implementing the dual chirality structure of the T4 figure-eight, whether or not the authors framed it that way.

### 5.2 Spinor Double Cover (Secondary Argument)

An alternative derivation: the T1 Möbius toroid requires 720° for closure — the spinor double cover of SO(3). This gives a factor of 2 between the 8 Malus projection channels and the number of independent measurements. The same factor appears in flux quantization Φ₀ = h/2e (Deaver & Fairbank 1961) and in the 720°/360° = 2 spinor ratio.

This argument correctly predicts 16 but requires an additional assumption — that each of the 8 lanes needs 2 independent measurements — which must be justified separately. On a T1 S², the double cover gets you to 16 lanes only if you assume both amplitude and phase must be sampled per lane. The dual chirality argument (Section 5.1) derives the factor of 2 from the physical structure of the figure-eight without this extra assumption, making it the stronger account.

Both arguments predict the same number. The dual chirality mechanism is more likely the operative one; the spinor double cover may be the topological reason the dual chirality structure exists in the first place.
