...
- 54 baryon/pentaquark masses at 0.531% MAPE with zero free parameters
...
(MAPE = 0.260% across 44 ground states, 0.531% across 54 including
...
**The Electron is T0 with a Möbius twist applied — T1.**  
The Möbius twist reorients the transverse Hamiltonian from a surface-running
path to a projecting path that crosses the boundary. This crossing accumulates
tension as mass. The Möbius boundary is asymmetric — its projection does not
cancel — leaving a permanent non-zero divergence. That is electric charge.
Charge is a topological property of the T1 Möbius twist, not of T0 itself.

*Correction from v3:* The electron is T1 (720° closure, spin-1/2, Möbius
twist), not T0. The photon is T0 (360° closure, spin-1, no twist). This
corrects the T0/T1 assignment throughout.

This is why the Schrödinger equation takes the form `iℏ ∂ψ/∂t = Hψ` —
time derivative on the left, Hamiltonian on the right. It is not a
coincidence. Time and the Hamiltonian are both T0. The equation describes
the relationship between the two paths on the same geometric object.

### 2.2 The Electron's Properties from T1 (Möbius-twisted T0)

| Property | Origin in T1 |
|----------|-------------|
| Charge | Möbius twist → asymmetric boundary → permanent ∇·E ≠ 0 |
| Spin 1/2 | Möbius T1 requires 720° closure → spinor double cover |
| Mass | Projecting Hamiltonian crosses boundary → tension cost |
| No color | No Y-junction → SU(3) does not apply |
| GUE statistics | Möbius twist → time-reversal symmetry broken |

### 2.2a The Photon's Properties from T0 (plain torus)

| Property | Origin in T0 |
|----------|-------------|
| No charge | Symmetric boundary → cancels exactly |
| Spin 1 | Plain torus, 360° closure |
| Massless | Surface-running Hamiltonian → zero boundary crossing |
| No color | No Y-junction |
| GOE statistics | No Möbius twist → time-reversal symmetric |

### 2.3 The Electron as T0 With a Möbius Twist — T1

The electron is **T0 with the Möbius twist applied** — it is not a separate
primitive object. The twist transforms the photon's surface-running Hamiltonian
into a projecting path that crosses the boundary, acquiring mass and charge.

| | T0 (photon) | T0 + Möbius twist = T1 (electron) |
|--|-------------|----------------------------------|
| Path orientation | Parallel to boundary | Perpendicular to boundary |
| Boundary projection | Zero → massless | Non-zero → mass |
| Closure | 360° spin-1 | 720° spin-1/2 |
| Statistics | GOE | GUE |
| Chirality | G = ±1 (circular polarization) | Spinor phase ±1 |
| Topology | Plain torus, C₀ cycle | Plain torus, C₁ cycle |
| Charge | None — χ̂=0, balanced | Yes — χ̂=−3m(m−1), monodromy accumulation |

The C₀/C₁ cycle distinction locks in the chirality behavior. These are
topological invariants of the Hamiltonian decomposition — proved exact by
the vortex chirality theorem. The two states G=+1 (RCP) and G=−1 (LCP)
inhabit separate chirality Hilbert spaces and do not interact in free
propagation. They meet only at the 84° vacuum seam.

The photon's C₀ balanced wave gives it polarization behavior. The electron's
C₁ monodromy gives it charge accumulation. Both emerge from the same T0
plain torus — the cycle type is the only distinction.

---

## 3. Chiral Hilbert Spaces

### 3.1 Two Directions of Push
...
by successive operations on T0.

| Level | Toroid | Construction | Physical object |
|-------|--------|-------------|-----------------|
| Substrate | Time string | T0, longitudinal Hamiltonian | Time — undeflected |
| Particle 0 | T0/C₀ | Balanced standing wave | Photon (spin-1, 360°, χ̂=0) |
| Particle 1 | T0/C₁ | Monodromy standing wave | Electron (spin-1/2, 720°, χ̂=−3m(m−1)) |
| Parabola X | T1 | First axis of parabola interior | Radial spatial degree of freedom |
| Parabola Y | T2 | Second axis of parabola interior | Azimuthal spatial degree of freedom |
| Parabola Z | T3 | Third axis of parabola interior | Longitudinal spatial degree of freedom |
| Both chiralities | T4+ | L+R parabolas paired | Antiquark+quark, generations, gluons |
...
### 4.3 T3: The Y-Junction

The third spatial dimension joins three T1 toroids at a Y-junction with
Z₃ symmetry. **T3 is the Hamiltonian of the triangle toroid** — the
same object seen as a path rather than a shape. The Y-junction is where
three quarks meet. The hard-bank transitions at each vertex are the color
exchange vertices of QCD. The 60°/30° decomposition of the junction is
the large/small scale structure of the strong force.

### 4.4 T4: Both Chiralities

For states requiring more than three spatial dimensions — heavy baryons
with more than three quarks, or antiparticle-particle bound states — both
...
**other chirality as its fourth dimension**. This is exactly what is
...
| Pentaquark | 0.502% | 35.64 | 5 |
| J=1/2 | 0.668% | 53.07 | 34 |
| J=3/2 | 0.298% | 18.66 | 20 |
| **ALL 54** | **0.531%** | **43.61** | **54** |
| *44 ground states* | *0.260%* | *13.45* | *44* |

Free parameter count: **0** — all constants derived from geometry.
Constituent masses derived from mock theta band-center angles (V8).
MAPE improves as parameters are *removed* — the opposite of overfitting.

---

## 6. The Strong Force — Geometry Without a Matrix
...
Time and the electron are the two Hamiltonians of T0. The photon is T0
with a Möbius twist, which reorients the electron's projecting path into
a massless surface-running wave. Stacking and joining T1 toroids builds
the three spatial dimensions. The Y-junction of three T1 toroids is the
Hamiltonian of the triangle toroid T3 — and also the three-quark baryon.
T4 objects borrow the other chirality as a fourth dimension, which is why
the weak force violates parity. Gravity adds tension to the time string.
...
Charge      is not a field — it is C₁ monodromy chirality accumulation at T0 boundary
Spin        is not a quantum number — it is the closure condition of the path
              spin-1 (T0) = 360°, spin-1/2 (T1) = 720°, spin-3/2 (T3) = 1080°
...
> *Time and the electron are its two Hamiltonians.*  
