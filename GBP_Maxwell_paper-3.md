...
### 2.5 Experimental Confirmation: Magnetic Flux is Quantized
...
**(Correction from earlier draft):** The charge-carrying particle is T0 with C₁ cycle topology — monodromy-induced chirality accumulation at the plain torus boundary. Both electron (C₁) and photon (C₀) are T0 plain torus waves. The distinction is cycle type, not a Möbius surface. The 720° closure of the electron comes from the C₁ monodromy condition (vortex chirality theorem: χ̂(C₁) = −3m(m−1)), not from a physical twist. The apparent spin is emergent wave behavior at the boundary.

∇·E = ρ/ε₀ is the continuum expression of T1 Möbius boundary imbalance.
...
1. **Unit conversion and amplitude:** c = cot(π/30) and Z₀ = tan(π/30) are in geometric units. The absolute field amplitude requires connecting Planck's constant h to the time-string tension T = c at the Planck-scale lane geometry. The K-K dilaton suppression (g₄₄ = constant) was the historical error that blocked this — see Section 5.4.
...
| Absolute amplitude | **(H) Pending** | h from time-string tension T=c |
