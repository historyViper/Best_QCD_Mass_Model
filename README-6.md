...
| Toroid | Closure | Spin |
|--------|---------|------|
| T0/C₀ cycle | 360° | 1 | Photon — χ̂=0, balanced |
| T0/C₁ cycle | 720° | 1/2 | Electron — χ̂=−3m(m−1), monodromy |
| T3 Y-junction | 3 × 720° | 3/2 | Delta, Omega |
| T4 double-helix | correlated pair | 2 | Entanglement only |

**Graviton forbidden:** spin-2 requires 180° which does not close any T0 cycle. Only exists as T4 correlated pair.

**Apparent spin is emergent:** Both electron and photon are T0 plain torus standing waves. The electron APPEARS to spin like a top because C₁ monodromy accumulates chirality with each boundary traversal. The photon APPEARS to rotate because its C₀ wave rotates. Neither has a physical Möbius twist — the distinction is cycle type (C₀ vs C₁), proved by the vortex chirality theorem (χ̂(C₀)=0, χ̂(C₁)=−3m(m−1)).

**Tunneling is lane-constrained:** Particles can exit before completing the full loop but only at valid mod-30 lane positions. Tunneling distance is quantized to lane spacing, not arbitrary.
