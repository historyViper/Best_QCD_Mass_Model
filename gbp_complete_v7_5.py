...
LAM = {
    ...
    # omega32h: ss+heavy J=3/2 — inverse phi rung (charm) or plain toroid (bottom)
    ('S1', 1.5, 'omega32h_c'): None,   # computed as LU/PHI in get_lam
    ('S1', 1.5, 'omega32h_b'): None,   # computed as LU in get_lam
}

def get_lam(sheet, J, T):
    if T == 'omega32h_c': return LU / PHI   # ss+charm J=3/2: inverse phi rung
    if T == 'omega32h_b': return LU          # ss+bottom J=3/2: plain toroid
    k = (sheet, J, T)
...
BARYON_CLASS = {
    ...
    'Omega_c*'    : ('S1', -1, 'sigma',  1, 'T1',   'omega32h_c'),  # v7.5: inverse phi rung
    ...
    'Omega_b*'    : ('S1', -1, 'sigma',  1, 'T1',   'omega32h_b'),  # v7.5: plain toroid
}
...
def predict_final(quarks, J, name=None):
    ...
    if rule in ('omega32h_c', 'omega32h_b'):
        # ss+heavy J=3/2: double-barrel-roll corner
        # charm: lam descends one phi rung (LU/phi) — second Möbius cycle winds back
        # bottom: lam stays at plain toroid (LU) — too heavy to complete second cycle
...
def print_summary(rows):
    ...
    print("=" * 70)
    print("GBP v7.5 — omega32h Branch (Omega_c*/Omega_b* double-barrel-roll fix)")
    print("=" * 70)
    ...
    print(f"  v7.5: Omega_c*→omega32h_c (lam=LU/phi), Omega_b*→omega32h_b (lam=LU)")
    print()
    ...
    print(f"  v7  baseline: 0.3029%")
    print(f"  v6  baseline: 0.4078%")
    print(f"  v5  baseline: 0.6365%")
    print(f"  v7.5 changes: omega32h_c/b branches for ss+heavy J=3/2 omega topology")
