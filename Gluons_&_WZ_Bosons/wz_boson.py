...
WEINBERG ANGLE — DERIVED:
  ...
  T3 corner bias correction:
  The T3 flip point (222.49°) is offset from the lane midpoint
  (216°) by 6.49° — the angular mismatch between the Y-path
  direction and the triangular toroid direction at each corner.
  This enters at half weight (one arm of two-arm confluence):
    θ_W = arctan(1/φ) - 6.49°/2 = 28.47°  (obs: 28.19°, Δ=0.28°)
  The 0.28° residual is within RG running of sin²(θ_W). (D) Derived.

OPEN PROBLEMS:
  H1. Absolute mass scale: requires connecting LU to electroweak VEV
...
def main():
    ...
    rows = [
        ...
        ('Double helix',  'Spin-2 massive',  'T2×T2',       '0',   'Conserved','(H) Speculative'),
    ]
    ...
    # ── Open problems ────────────────────────────────────────────────────
    _sec('OPEN PROBLEMS')
    print(f'  H1. Absolute mass scale')
    print(f'      dep_W and dep_Z are normalized (gluon energy = 1).')
    print(f'      To get GeV values: need to connect LU to electroweak VEV.')
    print(f'      Candidate: LU × something = M_W / M_Z_scale_factor')
    print()
    print(f'  H2. Weinberg angle — DERIVED (see Weinberg section above)')
    print(f'      θ_W = arctan(1/φ) - T3_bias/2 = 28.47°  (obs 28.19°, Δ=0.28°)')
    print(f'      Residual 0.28° consistent with sin²(θ_W) RG running.')
    print()
    print(f'  H3. Graviton channel')
    print(f'      Double-helix at T2_B = spin-2, massive, couples to T2_B.')
    print(f'      If real: gravity IS the residual of gluon-gluon confluence.')
    print(f'      Falsifiable: graviton mass would be ~ 2×dep_W in this model.')
    print()
    print(f'  H4. CKM mixing')
    print(f'      W± can also bridge T1_A↔T3_C (d↔u, s↔c etc.)')
    print(f'      The relative amplitudes of cross-generation transitions')
    print(f'      = the CKM matrix elements. Not modeled here.')
    print(f'      Direction: use cross-toroid lane products s²(la)×s²(lb)')
    print(f'      as CKM element proxies. d↔u: s²(11)×s²(19) = {s2(11)*s2(19):.6f}')
