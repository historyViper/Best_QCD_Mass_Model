...
def derive_geo_factor_heavy(quarks, chirality, sheet, cover, spin):
    ...
    if chirality == 'lambda':
        ...
        # sqrt(A×B) is the exact Malus result for equal-weight superposition.
        #
...
HYPERFINE_WHITELIST = set()  # v8.8: Sigma_b0 removed (Malus-IR gf); v9.8: Sigma_c+ removed (UDC triangle toroid cos²(45°)); v9.9: Sigma0 removed (base formula sufficient, delta_hyp was overcorrecting)
...
def reinforce(quarks, name=None):
    ...
    # UDC triangle toroid: cos²(45°) = 0.5 — sigma-type only (UD symmetric)
...
def print_summary(rows):
    ...
    print(f"    v9.8: MAPE=0.2427%  RMSE=14.76 MeV  UDC triangle toroid cos²(45°) — Sigma_c+ whitelist removed")
