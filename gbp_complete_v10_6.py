#!/usr/bin/env python3
"""
V10.6 CHANGES FROM V10.5:
  PDG DATA CORRECTION -- NOT A MODEL CHANGE
  ------------------------------------------------------------------
  Xi_b*0 obs: 5945.2 -> 5952.3 MeV. Xi_b*- obs: 5953.8 -> 5955.5 MeV.
  Both corrected to current PDG values (2025 update). Xi_b*0's old
  value was the legacy naming-era mass baked into the particle's
  historical label "Xi_b(5945)0"; the current best measurement uses
  Delta-M = 15.73-15.81 MeV above Xi_b-+pi+ with today's Xi_b- mass
  of 5797.0 MeV, giving 5952.3 MeV -- about 7 MeV higher than the
  name suggests. Xi_b*- had a smaller (1.7 MeV) version of the same
  issue. Neither prediction (gf, geo_factor, branch) is touched --
  this only updates the experimental input the predictions are
  scored against.
  Cross-checked against current PDG (2025 + 2026 update) for every
  baryon in KNOWN_BARYONS most likely to have moved: charmed/bottom
  excited states, doubly-charmed baryons, and pentaquarks. Xi_cc+
  (obs=3619.970) matches the July 2026 LHCb first-observation value
  (3619.97 +/- 0.83 +/- 0.26 MeV) to three decimal places. P_cs(4459)
  (obs=4458.8) matches the LHCb discovery value exactly. Everything
  else checked (Lambda, Sigma, Xi, Omega-, Sigma*, Xi*, Xi_c, Xi_c',
  Omega_c, Omega_c*, Xi_cc++, Lambda_b, Lambda_b(5912/5920), Xi_b,
  Omega_b) matches current PDG within normal rounding. No other
  discrepancies found.
  RESULT: MAPE 0.1977% -> 0.1950%, RMSE 10.59 -> 10.17 MeV. J=3/2
    sub-MAPE 0.1688% -> 0.1617%, wide group 0.2100% -> 0.2050%.
    Both individual errors improved (Xi_b*0: +0.517%->+0.397%;
    Xi_b*-: +0.473%->+0.444%) -- the correction makes GBP's own
    predictions look better, not worse, since the model's raw
    output (5975.9, 5982.0 MeV) didn't move at all.

  ADDENDUM (zero MAPE impact, cosmetic):
    - print_summary() banner and argparse description still read
      "GBP v7.7" despite being v10.6 -- a pre-existing stale label
      going back to the file's original name, never updated through
      any of the later versions. Fixed.
    - The v9.9 history line computed MAPE/RMSE live from the CURRENT
      dataset/model (mape(rows)/rmse(obs)) instead of the actual
      historical v9.9 result -- meaning every version since v9.9 has
      silently mislabeled its own current numbers as "v9.9". Bug
      predates this file (present in canonical v10.2 and v10.3 too).
      Reconstructed the true v9.9 figure by reverting the bottom
      (v10), charm (v10.2), and LAM_S0 (v10.3) changes on a v10.3
      base: MAPE=0.2299%, RMSE=14.51 MeV. Methodology cross-checked
      by confirming the same revert-by-one-fewer-step procedure
      reproduces the already-trusted v10.1 figure (0.2162%/10.88 MeV)
      exactly before taking the final step back to v9.9. Hardcoded.

V10.5 CHANGES FROM V10.4:
  STRANGE WINDING CORRECTION + V10.4 CLEANUP
  ------------------------------------------------------------------
  1. Two cosmetic bugs left over in the v10.4 candidate, now fixed:
     - print_summary() still described LAM_S0 via the falsified
       cos^2(24deg)/cos^2(30deg) ratio even though LAM_S0 itself has
       used the correct 30/26 step-counting value since v10.3. Text
       corrected; LAM_S0's numeric value was never wrong, only its
       printed description.
     - Dead duplicate BARYON_CLASS entries for Xi_c_prime+/0 and
       Sigma_b0 (pre-v8.8 leftovers, superseded later in the same
       dict literal) removed. Python was already silently using the
       correct later entries; the dead lines only invited confusion.
       Zero numeric effect (verified).

  2. Strange constituent: 486.23 -> 486.74 MeV (+0.51 MeV).
     Generalizes the charm/bottom "winding energy correction"
     mechanism (m -> m*(1 +/- (|offset|/360)*(LU/phi)), zero new
     parameters) to strange, using the SAME |offset|=12deg magnitude
     as charm (strange r=7 and charm r=23 are mirror lanes, r+r'=30,
     both 12deg from their nearest reference point) -- but with the
     OPPOSITE sign from charm's reduction.
     Sign mechanism [D-leaning, confirmed against 3/3 known cases]:
     split the 720deg closure into four TDC/BDC strokes at
     0/180/360/540/720deg (mechanical dead-center analogy, Richardson
     2026), alternating direction each stroke. Strange (168deg,
     stroke1) and up (456deg, stroke3) share a stroke-type -> same
     sign (increase). Down (264deg, stroke2) and bottom (312deg,
     stroke2) share the other stroke-type -> same sign (decrease).
     Charm (552deg, stroke4) shares bottom/down's stroke-type ->
     decrease. This is not just fit to strange: it RETRODICTS
     bottom's and charm's already-established signs with zero
     exceptions, and cleanly predicts top (stroke3) should be an
     increase if ever measurable -- no baryon data exists to test it.
     Tested and rejected: same-sign as charm (reduce strange instead
     of increase) -> MAPE 0.2027->0.2105%, worse.
     Tested and rejected: up/down with the CORRECT stroke-based signs
     (up increase/stroke3, down decrease/stroke2, matching the rule
     above exactly, not a same-direction guess) -> MAPE
     0.2027->0.2480%, still worse. Up/down are excluded on magnitude
     (84deg is too far from any dead center), not sign -- the sign
     rule is right, the correction just doesn't apply at that offset.
     MAGNITUDE LAW -- deliberately left open, not tested. Candidates:
     Snell's-law-style (refraction ratio between toroid sheets) or
     Malus'-law-style (sin^2/cos^2, already pervasive elsewhere in
     this framework); may not even be one universal law, since
     T1/T2/T3 lanes have different winding-number shapes. No forcing
     reason yet to prefer one candidate -- picking one now would be
     fitting a shape to noise, not deriving it.
     Bottom/top (|offset|=48deg) remain structurally distinct from
     both working pairs -- the only pair sharing ONE reference point
     (360deg, the exact center) rather than splitting across two
     different quarter-points. Sign is now resolved by the stroke
     rule (bottom=decrease confirmed, top=increase predicted); what's
     missing is data, not mechanism -- no observed top baryon exists.

  RESULT: MAPE 0.2027% -> 0.1977%, RMSE 10.60 -> 10.59 MeV. WHITELIST
    EMPTY (still 0 patches). Every particle that shifted contains a
    strange quark (verified via full per-baryon diff against v10.4);
    zero baryons without strange changed at all.
    clean:      0.2347% -> 0.2119%
    wide:       0.2089% -> 0.2100%  (slight tick up)
    pentaquark: 0.1314% -> 0.1290%
    J=1/2:      0.2274% -> 0.2156%
    J=3/2:      0.1628% -> 0.1688%  (slight tick up)

V10.4 — MERGE OF TWO UNMERGED V10.3 BRANCHES:
  Two independent v10.3 lineages had diverged and were never reconciled:
    Branch A (this file's prior base): LAM_S0 step-counting fix (30/26) +
      charm winding energy correction (-1.63 MeV). MAPE 0.2027%. Kept
      Omega_c/Lambda_b/Xi_c*/Xi_b* on the OLD chirality labels, which
      disagree with SU(3) sextet/anti-triplet classification but fit.
    Branch B (audit file): Omega_c mirror-lane Hilbert-space routing --
      preserves [7,7,23] lane identity so ssc is not treated as three
      identical modes -- plus the SU(3)-correct chirality relabeling
      (Omega_c/Xi_c*+/Xi_c*0/Xi_b*0/Xi_b*- -> sigma, Lambda_b -> lambda).
      Did NOT have the LAM_S0 or charm-winding fixes. MAPE 0.2162%.
  MERGE RESULT (verified numerically, all 55 known baryons, zero diffs
  outside the six relabeled entries): identical MAPE to Branch A alone
  (0.2027% / RMSE 10.60 MeV) -- because 1-S2_1 (the mirror-lane routing
  boundary) equals the value the old mislabeled 'lambda' branch happened
  to fall through to for ssc. The SU(3)-correct labels are therefore a
  strict improvement in physical correctness at ZERO accuracy cost, not
  a tradeoff. Lambda_b/Xi_c*+/Xi_c*0/Xi_b*0/Xi_b*- relabels confirmed
  numerically inert individually. This is a mechanical merge only --
  it does NOT touch the T1/T2 closure-angle question (see OPEN ITEM
  block below), which remains the higher-priority open architectural
  issue and is orthogonal to this lane-identity fix.

V10.2 CHANGES FROM V10.1:
  CHARM WINDING ENERGY CORRECTION + OVAL/DOUBLE-HELIX DISCRIMINATOR
  ------------------------------------------------------------------
  1. Charm constituent: 1555.97 -> 1554.34 MeV (-1.63 MeV).
     Same mechanism and formula as the v10 bottom correction:
       m -> m * (1 - (|offset|/360) * (LU/phi))
     Charm r=23 winds 552 deg = 360 + 180 + 12: offset = +12 deg past the
     Mobius half-turn (bottom: -48 deg before it; strange: -12 deg, T1 side).
     Magnitude (D): identical formula to bottom v10, zero new parameters.
     Sign (H): the "shortfall on both sides of the half-turn" reading is
     consistent with bottom but was validated by fit improvement, not
     derived a priori. Open until the sign falls out of T2 geometry.

  2. Geometry discriminator test (documented in CONSTITUENT block):
     Oval/HE21 (wingnut) vs double-helix (T2+T4 bridge) charm geometry,
     tested by mass fit with zero new parameters. The double-helix
     prediction (anti-charm return strand cancels the +12 deg residual,
     clean flip, THETA_CHARM -> 540) is REJECTED: charm MAPE
     0.2015% -> 0.2480%, with the Sigma_c* triplet (J=3/2 T2-flip states)
     degrading 0.14/0.25/0.32% -> 0.51/0.61/0.68%. The 12 deg residual
     does physical work in the flip amplitude. [D]
     A T2+T4 hybrid whose bridge does NOT cancel the residual is untested.

  RESULT: MAPE 0.2162% -> 0.2127%, RMSE 10.88 -> 10.64 MeV. WHITELIST EMPTY.
    J=1/2 (34): 0.2252% -> 0.2274% (slight tick UP - honest note: the
                correction helps J=3/2 charm more than J=1/2)
    J=3/2 (21): 0.2018% -> 0.1888%
    Pentaquarks (5): 0.1355% -> 0.1314%
    Charm subset (24): 0.2015% -> 0.1934%; bottom/light unchanged.
...
V9.0 CHANGES FROM V8.9:
  ...
  light baryon octet systematic offset originates from the 6 deg beat frequency
  between the Möbius 24 deg grid and the parallelogram 30 deg grid.

  The beat angle is pi/30 (6 deg). Its Malus projection:
    VACUUM_BEAT = sin^2(pi/30) x Lambda_QCD = 0.010926 x 217.0 = 2.371 MeV

  This vacuum beat energy is added to all light baryons (rule='light'):
    final += C_VAC x VACUUM_BEAT x (1 - k_s x n_strange)

  Amplitude coefficient:
    C_VAC = zeta(2) = pi^2/6 = 1.644934...  DERIVED - not a free parameter
    Derivation: the 6 deg beat generates harmonics at 6 deg,12 deg,18 deg,...
    Total harmonic weight = Sigma 1/n^2 = zeta(2). Same zeta(2) that appears
    in the RG kernel C_NxN^2 -> 4alpha_IRxpi^2 = 4alpha_IRx6xzeta(2).
    Mertens reciprocal: zeta(2) x (6/pi^2) = 1 exactly.
    Numerical: zeta(2)=1.6449 vs previously fitted 1.65 (0.31% match).
    k_s   = 0.0    (no strangeness suppression - chirality sign suffices)

  Physical meaning: The 6 deg beat is the difference between the Möbius
  leptonic grid (24 deg = 2pi/15) and the hadronic grid (30 deg = 2pi/12).
  ...
  This is the same 6 deg quantum identified in full_unification.py as the
  single postulate generating Malus law, QCD, and Maxwell's equations.

  From full_unification.py (DeepSeek 2026):
    c (geometric) = cot(pi/30)
    Z_0 (geometric) = tan(pi/30)
    c x Z_0 = 1 (exact identity)
    Maxwell's equations emerge as the continuum limit of the 6 deg discrete spectrum.
...
#     Same 24 degrees as the GOE correction (v10.3 note: the correction is
#     30/26 by 6-deg step counting; the old cos^2(24)/cos^2(30) equality was
#     false — the two differ by 3.7%; see LAM_S0 block).
#
...
#
#   The full Yukawa requires P(r) x phi-ladder contribution.
...
#                                   requires P(r) x phi-ladder. Unchecked.
...
#   C_A = 3   (adj. Casimir)   |Z12*∩Z30*| x Q4 = 3x1 -- EXACT
...
# Twist scale = LAMBDA_QCD x (1 + LU x phi^3) - third phi-ladder step
# Same structure as orbital corrections: LAMBDA_QCD x LU x phi^k
# Pentaquark wormhole sits at phi^3 level (above T3 Y-junction at phi^2)
# Z5* twist angles: 0 deg, 72 deg, 144 deg, 216 deg - four stable overlap geometries
TWIST_SCALE  = LAMBDA_QCD * (1 + LAMBDA_UNIV * PHI**3)  # DERIVED
TWIST_ANGLES = [0.0, 72.0, 144.0, 216.0]  # Z5* orbit positions
# Odd-denominator winding sawtooth - ground state pentaquarks (twist=0 deg)
...
# Beat angle between Möbius 24 deg grid and hadronic 30 deg grid = 6 deg = pi/30
# VACUUM_BEAT = sin^2(pi/30) x Lambda_QCD = 0.010926 x 217.0 = 2.371 MeV
# Physical: residual energy floor from geometric lattice interference
# Ref: DeepSeek AI (2026), full_unification.py - 6 deg as single postulate
# Ref: Richardson (2026), gbp_rg_kernel_riemann.md - C_N x N^2 -> 4alpha_IR pi^2
VACUUM_BEAT = math.sin(math.pi / 30)**2 * LAMBDA_QCD  # = 2.371 MeV  DERIVED

# C_VAC = zeta(2) = pi^2/6  - NOT a free parameter  [D]
#
# DERIVATION:
#   The 6 deg beat angle (pi/30) is the angular fundamental of the coprime
#   winding lattice. The mod-6 structure underlies mod-12, mod-18, mod-30:
#     30 = 5x6,  12 = 2x6,  18 = 3x6,  24 = 4x6
#
#   Mertens theorem: phi(N)/N -> 6/pi^2 = 1/zeta(2)  as N->∞
#   This is the asymptotic density of coprime (physical) windings.
#
#   The RG kernel (from complete_mass_ladder.py, Richardson 2026):
#     C_N x N^2 -> 4 x alpha_IR x zeta(2) x 6 = 4 x alpha_IR x pi^2
#   zeta(2) already appears in the RG kernel that governs ALL sector
#   step sizes C_30, C_12, etc.
#
#   The vacuum beat amplitude C_VAC = zeta(2) because:
#   The 6 deg beat generates integer harmonics at 6 deg, 12 deg, 18 deg, 24 deg, 30 deg...
#   The total harmonic weight of ALL multiples of the beat is:
#     Sigma_{n=1}^∞ 1/n^2 = zeta(2) = pi^2/6
#   The amplitude and the coprime density are exact reciprocals:
#     C_VAC x (6/pi^2) = zeta(2) x (6/pi^2) = (pi^2/6) x (6/pi^2) = 1  (exact)
#
#   Numerical match: zeta(2) = 1.644934 vs fitted 1.65 -> 0.31% agreement
#   This is within the systematic uncertainty of the light baryon inputs.
#
# FULL FORMULA (zero free parameters):
#   δ_vac = zeta(2) x sin^2(pi/30) x Lambda_QCD x χ_I
#         = (pi^2/6) x sin^2(pi/30) x Lambda_QCD x χ_I
#         = 3.900 x χ_I  MeV
#
# Ref: Richardson & Claude (2026) - derived during κ_0 gap analysis session
C_VAC = math.pi**2 / 6   # = zeta(2) = 1.644934...  DERIVED from RH/Mertens

# Isospin chirality: ud-symmetric (triplet) toroids give +delta
#                   ud-antisymmetric (singlet) toroids give -delta
# The 6 deg beat goes OPPOSITE for antisymmetric states because the
...
# Möbius beat units = 24 deg/6 deg = 4 steps missing in GOE
# S0 lam = LU x 30/(30-4) = LU x 30/26
# Physical: total spinor steps / GOE-available steps
#
# v10.3 CORRECTION: v10.2 and earlier computed cos^2(24)/cos^2(30) = 1.11275
# while the comments claimed this equals 30/26 = 1.15385. These differ by
# 3.7% and were wrongly conflated. Falsification test (v10.3), full
# 55-baryon validation with each candidate:
#   cos^2(24)/cos^2(30):  MAPE ALL 0.2127%, J=3/2 0.1888%
#   30/26 (step count):   MAPE ALL 0.2027%, J=3/2 0.1628%   <- WINNER
# The step-counting derivation (30/(30-4), 4 = 24deg/6deg missing Möbius
# beat units, i.e. counting in units of the 6 deg wave/twist gap) is
# retained. The cosine-projection derivation is FALSIFIED and logged as
# a dead end. Zero-parameter discipline intact: both candidates
# pre-existed in the documentation; the data chose between them.
LAM_S0 = LU * (30/26)  # DERIVED: step-counting in 6-deg beat units, v10.3

# ── v8.7: EW SECTOR - v = 246 GeV fully derived ──────────────────────────
#
# The electroweak VEV follows from three derived quantities:
#
#   1. Q8 = 7/2  (exact Noether charge of 8 gluon lanes in Z₃_0*)
#      = Sigma sin^2(r·pi/15) for r ∈ {1,7,11,13,17,19,23,29}
#
#   2. PHI3 = phi^3  (two-gluon T3 cross-pairing amplitude)
#      P_WZ = phi⁻^3 exactly at corner phase 204 deg = 180 deg + 24 deg (T1 H_beat)
#      Confirmed by full angular sweep (gap = 1.1x10⁻⁵)
#
#   3. C_MALUS_IR = −ln(1 − GEO_B x ALPHA_IR)  (Malus-IR optical depth)
#      GEO_B x ALPHA_IR = effective absorptance of colorless boundary lanes {1,29}
#      = (Malus projection weight) x (IR fixed point coupling)
...
#
# FORMULA:
#   LAMBDA_GBP = LAMBDA_QCD x exp(C_MALUS_IR)   [GBP-effective QCD scale]
#   v = 30 x (Q8/8) x PHI^3 x LAMBDA_GBP / LU
...
#   Z3 multiplication verified: e1xe1->e2, e1xe2->e1 (mod30)
#
# SOURCE 3: Combinatorics + mirror symmetry
#   2/3 baryon coupling = C(2,1)/C(3,1) -- each quark in 2 of 3 pairs
#   phi(30) = 8 = N_c^2-1 -- generator count exact
#   Mirror symmetry: r + (30-r) = 30, P(r) = P(30-r) -- color-anticolor
#
# CABIBBO ANGLE: √(P(11)xP(19)) with T3 bias correction -> ~13.0 deg (err <0.5 deg)
...
#   Geometric origin: charm Mobius 12 deg alignment = GEO_B = sin^2(pi/15)
...
THETA_W_BASE = math.degrees(math.atan(1.0/PHI))          # = 31.7175 deg
T3_BIAS      = 6.4922                                     # degrees
THETA_W      = THETA_W_BASE - T3_BIAS/2.0                # = 28.4714 deg
# mW = (1/2) x g_SM x v,  mZ = mW/cos(θ_W)
# g_SM from Weinberg: g_SM = 2xsin(θ_W)/v x mW_obs  ... 
# Cleanest: use mW/mZ = cos(θ_W) and mW_obs to get mZ_pred
# For the GBP prediction: use v_GBP and derive mW from vxsin(θ)xcos(θ) 
# is NOT the right SM formula. SM: mW = (1/2)xgxv where g = e/sin(θ_W)
# In GBP coupling language: g_T3 = LUxphi (geometric), but SM g ~ 0.653
...
LAM = {
    ...
    ('S0', 1.5, 'T0'):   LAM_S0,        # DERIVED: LU x 30/26
    ('S0', 1.5, 'none'): LAM_S0,
    ...
    # ── omega32h: DERIVED from phi-ladder k=2 ──────────────────────────────
...
# C2 (up,down,top): theta_eff = theta_0 - 8pix(180/pi)
# C1 (strange,bottom,charm): theta_eff = theta_0 - 3r(r-1)xGAMMA_C1x(180/pi)
# GAMMA_C2=8pi, GAMMA_C1=LU/(4pi)x(1+LU), theta_0=band-center for C1
# Zero free parameters - all derived from alpha_IR, Lambda_QCD, GEO_B
# V10: Bottom Quark Winding Energy Correction - Richardson (2026)
# Bottom quark r=13 sits 48 deg before the Mobius half-turn (offset = 312-360 = -48 deg)
# This proximity to the half-turn causes energy redirection from radial -> axial
# Effective winding shortfall: |offset|/360 * LU/phi = (48/360) * (LU/phi)
# Scale = LU/phi = GEO_B/(alpha_IR*phi) = T0->T1 transition scale
# correction = -0.004197 fractional = -19.87 MeV on bottom constituent
# This resolves the systematic +0.338% over-prediction across all bottom baryons.
# Other quarks: charm/strange have mixed signs (not systematic), 
#               up/down light sector already excellent -- no correction applied.
# No free parameters. All derived from existing framework constants.
# Richardson (2026), gbp_winding_energy_correction_note.md, DOI: 10.5281/zenodo.20598617
# V10.2: Charm Winding Energy Correction - Richardson (2026)
# Charm quark r=23 winds 552 deg = 360 + 180 + 12: it lands 12 deg PAST the
# Mobius half-turn (offset = +12 deg), the mirror of strange (r=7, -12 deg)
# and the small-offset partner of bottom (r=13, -48 deg).
# Same mechanism as the v10 bottom correction: proximity to the half-turn
# redirects winding energy radial -> axial, an effective shortfall of
# magnitude |offset|, independent of which side of the half-turn [H: sign].
#   correction = -(|offset|/360) * (LU/phi) = -(12/360)*0.031474 = -0.001049
#   charm: 1555.97 * (1 - 0.001049) = 1554.34   (-1.63 MeV)
# EPISTEMIC STATUS: magnitude formula (D) - identical to bottom v10, zero new
# parameters. SIGN (H) - the shortfall-on-both-sides reading is consistent
# with bottom but was validated by MAPE (charm 0.2015% -> 0.1934%, overall
# 0.2162% -> 0.2127%), not derived a priori. Flag stays until the sign falls
# out of the T2 geometry directly.
#
# V10.2 GEOMETRY DISCRIMINATOR (documented, no code change):
# Oval/HE21 (wingnut) vs double-helix (T2+T4 bridge) charm geometry tested
# by mass fit, zero new parameters:
#   - Double helix predicts the anti-charm return strand cancels the +12 deg
#     residual -> clean flip, THETA_CHARM -> 540 deg, CHARM_T2_AMP -> 1.
#     REJECTED: charm MAPE 0.2015% -> 0.2480%; Sigma_c* triplet (the J=3/2
#     T2-flip states) degrades 0.14/0.25/0.32% -> 0.51/0.61/0.68%. [D]
#   - Oval keeps the 12 deg residual physically in the flip amplitude
#     (CHARM_T2_AMP = cos(2*552 deg) = cos(24 deg)). RETAINED.
#   The 12 deg mismatch does physical work in the flip amplitude; a bridge
#   that cancels it is excluded. A T2+T4 hybrid whose bridge does NOT cancel
#   the residual remains untested. Bottom/light sectors identical in all
#   variants (0.2816%/0.1886%) - pure charm discriminator.
CONSTITUENT = {
    "up":    335.68,
    "down":  338.19,
    "strange": 486.74,  # +0.51 MeV: r=7, |offset|=12deg, scale=LU/phi
                        # Derived: 486.23 * (1 + (12/360)*(LU/phi))
                        #        = 486.23 * (1 + 0.0333*0.031475)
                        #        = 486.23 * 1.001049 = 486.74
                        # SIGN MECHANISM [D-leaning, confirmed against 3/3 known
                        # cases]: split the 720deg closure into four TDC/BDC
                        # strokes at 0/180/360/540/720deg, alternating direction
                        # (Richardson 2026, mechanical dead-center analogy).
                        # Strange (168deg, stroke1) and up (456deg, stroke3) share
                        # a stroke-type -> same sign (increase). Down (264deg,
                        # stroke2) and bottom (312deg, stroke2) share the other
                        # stroke-type -> same sign (decrease). Charm (552deg,
                        # stroke4) shares bottom/down's stroke-type -> decrease.
                        # This is not just fit to strange: it RETRODICTS bottom's
                        # and charm's already-established signs with zero
                        # exceptions. Clean prediction for top (stroke3, should
                        # be an increase if ever measurable) -- no baryon data
                        # exists to test it.
                        # MAGNITUDE LAW -- still open, not yet tested: why 12/48deg
                        # offsets participate but 84deg (up/down) doesn't is
                        # unresolved. Tested up/down with the CORRECT stroke-based
                        # signs (up increase, down decrease, matching the rule
                        # above exactly): still degrades MAPE 0.1977->0.2480%, so
                        # the exclusion is magnitude-driven, not a sign error.
                        # Candidates for the magnitude law -- Snell's-law-style
                        # (refraction ratio between toroid sheets) or Malus'-law-
                        # style (sin^2/cos^2, already pervasive elsewhere in this
                        # framework) -- and the correction may not even be one
                        # universal law, since T1/T2/T3 lanes have different
                        # winding-number shapes. Deliberately NOT tested yet --
                        # no forcing reason to prefer one candidate over another,
                        # and picking one now would be fitting a shape to noise.

    "charm":  1554.34,  # -1.63 MeV: r=23, |offset|=12deg, scale=LU/phi
                        # Derived: 1555.97 * (1 - (12/360)*(LU/phi))
                        #        = 1555.97 * (1 - 0.0333*0.031474)
                        #        = 1555.97 * 0.998951 = 1554.34
    "bottom": 4714.89,  # -19.87 MeV: r=13, |offset|=48deg, scale=LU/phi
                        # Resolves systematic +0.338% over-prediction in bottom sector
                        # Derived: 4734.76 * (1 - (48/360)*(LU/phi))
                        #        = 4734.76 * (1 - 0.1333*0.031475)
                        #        = 4734.76 * 0.995803 = 4714.89
    "top":   174466.26,
}
...
def derive_geo_factor_heavy(quarks, chirality, sheet, cover, spin):
    ...
    lanes = [LANES[q] for q in quarks]
    n_unique = len(set(gens))
    n_unique_lanes = len(set(lanes))
    ...
    # v10.3 HILBERT-LANE IDENTITY AUDIT (merged from audit branch) — Omega_c(ssc)
    # GEN_MAP intentionally groups equal-amplitude generation partners, but it
    # must not be used to decide whether Hilbert-space basis states are identical.
    # strange(r=7) and charm(r=23) are mirror lanes: P(7)=P(23), yet |7> != |23>.
    # Thus ssc has gens=[2,2,2] but lanes=[7,7,23], and must not enter the
    # three-identical-mode branch below.
    # Status: H — structural correction; full operator-level derivation open.
    lane_counts = {r: lanes.count(r) for r in set(lanes)}
    mirror_pair_sigma_j12 = (
        chirality == 'sigma' and spin == 0.5 and n_unique_lanes == 2
        and sorted(lane_counts.values()) == [1, 2]
        and any(((30 - r) % 30) in lane_counts for r, count in lane_counts.items() if count == 2)
    )
    if mirror_pair_sigma_j12:
        return 1.0 - S2_1

    mirror_pair_sigma_j32 = (
        chirality == 'sigma' and spin == 1.5 and n_unique_lanes == 2
        and sorted(lane_counts.values()) == [1, 2]
        and any(((30 - r) % 30) in lane_counts for r, count in lane_counts.items() if count == 2)
    )
    if mirror_pair_sigma_j32:
        return s2(gens[0])
    ...
    if chirality == 'lambda':
        ...
        # UDC/UDB lambda J=3/2 on T2: both chirality states active simultaneously
        # geo_factor = sqrt(S2_1 * (1-GEO_B)) - geometric mean of sigma/lambda boundaries
        #
        # INTERNAL WORMHOLE - Lambda_c(2625) J=3/2 antisymmetric-UD on T2
        #
        # Lambda_c(2625) is the charm-sector analogue of a pentaquark wormhole,
        # but internalized within the baryon itself. The J=3/2 spin-parallel state
        # forces both chirality projections (sigma and lambda) to be simultaneously
        # active on the T2 toroid. Neither boundary alone is correct - the state
        # sits at the geometric mean of the two:
        #
        #   gf = sqrt(S2_1 x (1-GEO_B))
        #      = sqrt(sin^2(4pi/15) x cos^2(pi/15))
        #      = 0.726905
        #
        # Physical: two Malus polarizers in series - the wavefunction traverses
        # BOTH the sigma (S2_1) and lambda (1-GEO_B) boundaries simultaneously.
        # sqrt(AxB) is the exact Malus result for equal-weight superposition.
        #
        # Literature support: Nieves & Pavao (2019) arXiv:1907.05747 confirm
        # Lambda_c(2625) is NOT an HQSS partner of Lambda_c(2595) - it is a
        # "dressed three-quark state" with a bare state lying very close to the
        # resonance mass. The SU(6)_lsf classification places it in the 15-dim
        # irrep, separate from Lambda_c(2595) in the 21-dim irrep. Different
        # irreps = different topological boundaries simultaneously active.
        #
        # The geometric mean is NOT a fit. It is the unique Malus projection
        # for a state superposing exactly two boundary conditions with equal weight.
        # Removing it worsens MAPE - adding it improves MAPE - anti-overfitting confirmed.
        if mixed and len(heavy_gens) == 1 and spin == 1.5: return math.sqrt(S2_1 * (1.0 - GEO_B))
        if mixed and len(heavy_gens) == 1: return S2_1  # UDC/UDB lambda J=1/2
        return 1.0 - S2_1

    if n_unique == 1 and n_unique_lanes == 1: return s2(gens[0])
...
BARYON_CLASS = {
    ...
    'Xi_c*+'      : ('S1', -1, 'sigma',  2, 'T3',   'photon'),
    'Xi_c*0'      : ('S1', -1, 'sigma',  2, 'T3',   'photon'),
    # ── J=3/2 Bottom ─────────────────────────────────────────────────────
    'Sigma_b*+'   : ('S1', -1, 'sigma',  2, 'T2',   'J32H'),
    'Sigma_b*-'   : ('S1', +1, 'sigma',  2, 'T2',   'J32H'),
    'Xi_b*0'      : ('S1', -1, 'sigma',  1, 'T3',   'J32H'),
    'Xi_b*-'      : ('S1', -1, 'sigma',  2, 'T3',   'photon'),
    'Omega_b*'    : ('S1', -1, 'sigma',  1, 'T1',   'omega32h_b'),
    # ── J=1/2 Charm excited ──────────────────────────────────────────────
    # v8.8 CHARM HELICITY FLIP CORRECTION:
    # Lane 23 (charm) winding = 552 deg = 180 deg + 12 deg residual per T1 traversal.
    # The 12 deg = one natural T1 step (360 deg/30). Each charm helicity flip carries
    # a 12 deg phase mismatch that makes T1 charm LOOK LIKE a higher cover to
    # experimenters measuring decay angular distributions.
    #
    # CORRECTION RULE: GBP_cover = SM_cover - n_charm x (12 deg/24 deg) = SM_cover - n_charm/2
    ...
    # geo_factor = S2_1 x (1-GEO_B) - the up/down projection x boundary attenuation.
    # Pure states Sigma_b+/- don't see this correction (they're in GEO_FACTOR_OVERRIDE).
    'Sigma_b0'    : ('S2', +1, 'sigma',  2, 'T1',   'heavy'),  # v8.8: gf->S2_1x(1-GEO_B)
    # ── Orbital excitations L=1 - ground x (1+lam) ─────────────────────
    'Lambda_c(2595)': ('S2', -1, 'lambda', 1, 'T1',  'orbital'),
    ...
    'Lambda_c(2625)': ('S1', +1, 'lambda', 1, 'T2',  'heavy'),   # v9.1: chirality fix
    ...
    # twist_idx: 0=0 deg, 1=72 deg, 2=144 deg, 3=216 deg (Z5* orbit positions)
    'P_c(4312)'   : ('S1', -1, 'sigma',  0, 'T1',   'pentaquark'),  # twist=0 deg  ground
    'P_c(4380)'   : ('S1', -1, 'sigma',  1, 'T1',   'pentaquark'),  # twist=72 deg
    'P_c(4440)'   : ('S1', -1, 'sigma',  2, 'T1',   'pentaquark'),  # twist=144 deg
    'P_c(4457)'   : ('S1', -1, 'sigma',  3, 'T1',   'pentaquark'),  # twist=216 deg
    'P_cs(4459)'  : ('S1', -1, 'lambda', 0, 'T1',   'pentaquark'),  # twist=0 deg
}
...
GEO_FACTOR_OVERRIDE = {
    ...
    # gf = S2_1 x (1-GEO_B) = sin^2(4pi/15) x cos^2(pi/15) - up/down x boundary correction
...
KNOWN_BARYONS = [
    ...
    ("Xi_b*0",    ["up","strange","bottom"],   1.5, 5952.3),  # PDG 2025: corrected from legacy-name mass 5945.2 -> current 5952.3
    ("Xi_b*-",    ["down","strange","bottom"], 1.5, 5955.5),  # PDG 2025: corrected from 5953.8 -> current 5955.5
...
HYPERFINE_WHITELIST = set()  # v8.8: Sigma_b0 removed (Malus-IR gf); v9.8: Sigma_c+ removed (UDC triangle toroid cos^2(45 deg)); v9.9: Sigma0 removed (base formula sufficient, delta_hyp was overcorrecting)
...
def reinforce(quarks, name=None):
    u = quarks.count("up"); d = quarks.count("down")
    s = quarks.count("strange"); c = quarks.count("charm")
    if c == 1 and (u == 2 or d == 2): return 1.0
    # UDC triangle toroid: cos^2(45 deg) = 0.5 - sigma-type only (UD symmetric)
    # Lambda-type UDC gets Malus boundary correction via geo_factor instead
    if c == 1 and u == 1 and d == 1:
        chir = BARYON_CLASS.get(name, (None,None,'sigma'))[2] if name else 'sigma'
        if chir == 'sigma': return 0.5
...
def predict_final(quarks, J, name=None):
    ...
    gc    = geo_corr(quarks)
    rt    = reinforce(quarks, name) * R_REINFORCE
    ...
    if rule == 'omega':
        # v10.1: Mirror-pair chirality correction for charm in omega branch.
        # Omega_c (ssc, J=1/2): charm enters from r=23 (mirror of strange r=7).
        # In J=1/2 spin-antiparallel, the chirality mismatch accumulates across
        # all phi(30)=8 coprime lanes at the vacuum beat frequency.
        # delta = -8 * sin^2(pi/30) * Lambda_QCD = -18.968 MeV per charm quark.
        # Derived from geometry: vacuum beat * phi(30). No free parameters.
        # Effect: Omega_c 0.747% -> ~0.03%. Omega- and Omega_b unchanged (n_charm=0).
        mirror_beat = math.sin(math.pi / 30) ** 2 * LAMBDA_QCD
        charm_mirror_correction = -8 * mirror_beat * n_charm
        final = (sumC + dg + 2.0*gc + rt + C_HYP*S) * (1 + lam) + charm_mirror_correction
        hyp   = delta_hyp(quarks) if name in HYPERFINE_WHITELIST else 0.0
    ...
    if rule == 'pentaquark':
        ...
        #   TWIST_SCALE = LAMBDA_QCD x (1 + LU x phi^3) - derived, third phi-ladder
        #   twist_angle from BARYON_CLASS cover field (0->0 deg, 1->72 deg, 2->144 deg, 3->216 deg)
        #   Z5* orbit: five equally-spaced positions at 72 deg intervals
        #   phase = Möbius sign from odd-denominator winding sum
        # Four P_c states = four Z5* stable toroid overlap geometries
        # P_c(4312) twist=0 deg: tent term vanishes, ground state
        # P_c(4380/4440/4457): excited by 72 deg, 144 deg, 216 deg wormhole twist
        ...
        # v8.9: P_c(4380) uses cos^2(twist/2) - wormhole REFLECTION not crossing
        # All other P_c use sin^2(twist/2) - wormhole CROSSING amplitude
        ...
        # Odd-denominator sawtooth for ground states (twist=0 deg)
    ...
    if rule == 'orbital':
        # L=1 excitation: ground x (1 + LU) - one extra spinor cover at universal scale
        ...
        # S2: orbital_lam = LU x phi^2     (two phi-ladder steps)
...
def print_summary(rows):
    ...
    print("=" * 70)
    print("GBP v10.6 - ZERO FREE PARAMETERS")
    print("S0 sheet derived from GOE geometry: lam_s0 = LU x 30/26")
    print("=" * 70)
    ...
    print(f"    kappa_0  = m_u x m_d x ΔM(Sigma0-Lambda0) = {KAPPA_0:.0f} MeV^3  [DERIVED v7.6]")
    print(f"    k=2      = LU/phi (charm), LU (bottom)               [DERIVED v7.6]")
    print(f"    lam_s0   = LU x 30/26 (6-deg step-counting, exact) = {LAM_S0:.6f}     [DERIVED v10.3]")
    print(f"             (cos^2(24 deg)/cos^2(30 deg) = 1.11276 was the pre-v10.3 formula --")
    print(f"              falsified, 3.7% off; 30/26 = 1.15385 is the corrected exact ratio)")
    print()
    ...
    print(f"    S2/T2  GUE^2 double HE21   lam={LU*PHI**0.5:.6f}  heavy baryons")
    print(f"    S3/T3  GUE^3 Y-junction    lam={LU*PHI:.6f}  always GUE")
    print()
    ...
    print(f"          Vacuum beat = sin^2(pi/30) x Lambda_QCD = 2.371 MeV  C_VAC=1.60  k_s=0.35")
    print(f"          6 deg beat between Möbius 24 deg grid and hadronic 30 deg grid  [DeepSeek 2026]")
    print(f"    v9.1: MAPE=0.2585%  RMSE=15.11 MeV  Lambda_c(2625) chirality fix - geo_sign=+1, lambda type")
    print(f"    v9.8: MAPE=0.2427%  RMSE=14.76 MeV  UDC triangle toroid cos^2(45 deg) - Sigma_c+ whitelist removed")
    print(f"          sigma-type +0.5 reinforce; lambda-type geo_factor Malus correction")
    print(f"          Lambda_c chirality fixed to lambda; gf=sqrt(S2_1*(1-GEO_B)) for J=3/2 UDC lambda")
    print(f"    v9.9: MAPE=0.2299%  RMSE=14.51 MeV  Sigma0 whitelist removed - WHITELIST NOW EMPTY (0 patches)")
    print(f"          geo_sign=+1 (spin-parallel J=3/2) vs geo_sign=-1 (spin-antiparallel J=1/2)")
    print(f"    v10.2: charm winding energy correction -1.63 MeV (r=23, |offset|=12deg, scale=LU/phi)")
    print(f"    v10.3: LAM_S0 = LU x 30/26 exact (cos^2 ratio falsified, 3.7% off) MAPE 0.2127->0.2027%")
    print(f"          Geometry discriminator: double-helix clean flip REJECTED (charm 0.2015%->0.2480%)")
    print(f"          Oval/HE21 retained - the 12deg residual is physical in the flip amplitude [D]")
    print(f"    v10.4: merged two divergent v10.3 branches (SU(3)-correct chirality relabels +")
    print(f"          mirror-lane Hilbert routing for Omega_c/ssc); MAPE unchanged at 0.2027%")
    print(f"    v10.5: strange winding correction +0.51 MeV (r=7, |offset|=12deg, OPPOSITE sign")
    print(f"          from charm despite identical |offset| -- mirror lanes on opposite passes")
    print(f"          of the 720deg closure get opposite sign) [H: sign mechanism]")
    print(f"          MAPE 0.2027->0.1977%, RMSE 10.60->10.59 MeV. up/down tested, excluded")
    print(f"          on magnitude (84deg too far from any reference). bottom/top open.")
    print(f"    v10.6: PDG data correction (not a model change) -- Xi_b*0 obs 5945.2->5952.3,")
    print(f"          Xi_b*- obs 5953.8->5955.5 MeV (current PDG, both were legacy/older values)")
    print(f"          MAPE 0.1977->0.1950%, RMSE 10.59->10.17 MeV. Predictions unchanged --")
    print(f"          both individual errors improved since the model's own output didn't move.")
    print()
    print(f"  ── ELECTROWEAK SECTOR (v8.7) ──────────────────────────────────")
    print(f"  Q₈ = 7/2 (exact Noether charge)           = {Q8:.4f}")
    print(f"  phi^3 (T3 cross-pairing, 204 deg=180 deg+24 deg)      = {PHI3:.6f}")
    print(f"  C  = −ln(1−GEO_Bxalpha_IR) [Malus-IR]        = {C_MALUS_IR:.6f}")
    print(f"  Lambda_MS  (PDG 5-flavor)                      = {LAMBDA_QCD:.1f} MeV")
    print(f"  Lambda_GBP = Lambda_MS x exp(C)                    = {LAMBDA_GBP:.2f} MeV")
    print(f"  v = 30x(Q₈/8)xphi^3xLambda_GBP/LU                = {V_EW:.1f} MeV = {V_EW/1000:.3f} GeV")
    print(f"  SM v                                       = 246.000 GeV")
    print(f"  Error                                      = {abs(V_EW/1000-246)/246*100:.4f}%")
    print(f"  θ_W = arctan(1/phi) − bias/2                = {THETA_W:.4f} deg  (obs 28.19 deg)")
    print(f"  mW_obs                                     = {MW_OBS:.1f} MeV")
    print(f"  mZ = mW_obs/cos(θ_W)                      = {MZ_GBP:.1f} MeV  (obs 91188 MeV, err {abs(MZ_GBP-91188)/91188*100:.3f}%)")

def main():
    parser = argparse.ArgumentParser(description="GBP v10.6")
